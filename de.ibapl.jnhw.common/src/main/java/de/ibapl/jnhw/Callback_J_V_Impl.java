...
public abstract class Callback_J_V_Impl extends Callback_J_V implements NativeCallToJava {

    private final static Logger LOG = Logger.getLogger("d.i.j.c.Callback_J_V");

    private static final WeakReference<Callback_J_V_Impl> refs[];

    public static Callback_J_V_Impl find(NativeFunctionPointer callbackPtr) {
        for (WeakReference<Callback_J_V_Impl> wr : refs) {
            Callback_J_V_Impl cb = wr.get();
            if (cb != null) {
    ...
    public static int callbacksAvailable() {
        int result = 0;
        for (WeakReference<Callback_J_V_Impl> ref : refs) {
            if (ref.get() == null) {
                result++;
            }
        }
    ...
    public Callback_J_V_Impl() {
        super(Callback_J_V_Impl::aquire);
    }

    private static void trampoline(final int index, final long value) {
        try {
            final Callback_J_V_Impl ref = refs[index].get();
            if (ref == null) {
    ...
    private static synchronized NativeAddressHolder aquire(Callback_J_V_Impl cb) {
        for (int i = 0; i < refs.length; i++) {
            if (refs[i].get() == null) {
