 ...
 * @author aploese
 */
public class JniCallbackFactory__V__MA extends CallbackFactory__V__MA {
    ...
    private MemorySegment registerCallBack(int index) {
        final long result = registerCallBack(index, JniCallbackFactory__V__MA.class, "trampoline_" + index);
        if (result == 0) {
    ...
    @Override
    protected void release0(Callback__V__MA cb) {
        for (int i = 0; i < MAX_CALL_BACKS; i++) {
            if (REFS[i] == cb) {
                REFS[i] = null;
