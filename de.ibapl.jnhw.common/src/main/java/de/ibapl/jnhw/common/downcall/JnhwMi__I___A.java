 ...
 * @author aploese
 */
public class JnhwMi__I___A extends JnhwMethodInvoker implements JnhwMh_sI___A, JnhwMh__B___A, JnhwMh_uL___A {

    public JnhwMi__I___A(SymbolLookup symbolLookup, String name) {
        super(symbolLookup, name, FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
    }

    @Override
    public int invoke_sI___P(Pointer<?> arg1) {
        try {
    ...
    @Override
    public boolean invoke__B___P(Pointer<?> arg1) {
        try {
            return ConversionsNative2Java.int32_t_TO_boolean(
                    (int) methodHandle.invokeExact(arg1.toAddressable()));
        } catch (NullPointerException npe) {
            throw npe;
        } catch (Throwable t) {
            throw createRuntimeExceptionInvoke(t);
