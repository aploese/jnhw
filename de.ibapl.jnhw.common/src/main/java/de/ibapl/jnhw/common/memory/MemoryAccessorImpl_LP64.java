 ...
 * @author aploese
 */
public final class MemoryAccessorImpl_LP64 extends AbstractMemoryAccessorImpl {
    ...
    public MemoryAccessorImpl_LP64(ByteOrder byteOrder) {
        super(byteOrder);
        LAYOUT__LONG = ValueLayout.JAVA_LONG.withOrder(byteOrder);
        LAYOUT__UNSIGNED_LONG = ValueLayout.JAVA_LONG.withOrder(byteOrder);
    }

    public MemoryAccessorImpl_LP64(ByteOrder byteOrder, long alignmentBits) {
        super(byteOrder, alignmentBits);
        LAYOUT__LONG = ValueLayout.JAVA_LONG.withOrder(byteOrder).withBitAlignment(alignmentBits);
        LAYOUT__UNSIGNED_LONG = ValueLayout.JAVA_LONG.withOrder(byteOrder).withBitAlignment(alignmentBits);
    }
    ...
    public String intptr_t_AsHex(MemorySegment mem, long offset) {
        return String.format("0x%016x", intptr_t(mem, offset).address().toRawLongValue());
    }

    @Override
    public String uintptr_t_AsHex(MemorySegment mem, long offset) {
        return String.format("0x%016x", uintptr_t(mem, offset).address().toRawLongValue());
    }
