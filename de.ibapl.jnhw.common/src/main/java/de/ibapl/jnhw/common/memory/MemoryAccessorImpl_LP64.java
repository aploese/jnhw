...
public final class MemoryAccessorImpl_LP64 extends AbstractMemoryAccessorImpl {
    ...
    public MemoryAccessorImpl_LP64(ByteOrder byteOrder, long byteAlignment) {
        super(byteOrder, byteAlignment);
        LAYOUT__LONG = ValueLayout.JAVA_LONG.withOrder(byteOrder).withByteAlignment(byteAlignment);
        LAYOUT__UNSIGNED_LONG = ValueLayout.JAVA_LONG.withOrder(byteOrder).withByteAlignment(byteAlignment);
    }
    ...
    public String intptr_t_AsHex(MemorySegment mem, long offset) {
        return String.format("0x%016x", intptr_t(mem, offset).address());
    }

    @Override
    public String uintptr_t_AsHex(MemorySegment mem, long offset) {
        return String.format("0x%016x", uintptr_t(mem, offset).address());
    }
