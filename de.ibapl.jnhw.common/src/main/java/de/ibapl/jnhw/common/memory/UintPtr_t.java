...
public class UintPtr_t<D extends Pointer> extends OpaqueMemory {
    ...
    public static <T extends Pointer> UintPtr_t<T> allocateNative(MemorySession ms) {
        return new UintPtr_t<>(MemorySegment.allocateNative(DATA_TYPE.SIZE_OF, ms), 0);
    }

    public static <T extends Pointer> UintPtr_t<T> ofAddress(MemoryAddress baseAddress, MemorySession ms) {
        return new UintPtr_t<>(MemorySegment.ofAddress(baseAddress, DATA_TYPE.SIZE_OF, ms), 0);
    }
    ...
    @Deprecated // Optional??
    public <O extends OpaqueMemory> O getAsOpaqueMemory(OpaqueMemoryProducer<O, UintPtr_t<D>> opaqueMemoryProducer, MemorySession ms) {
    ...
    public void set(D value) {
        MEM_ACCESS.uintptr_t(memorySegment, 0, value.toAddressable());
        cached = value;
    }
