 ...
 */
//@TODO write test
public class StructLayoutOnTheFlyFactory implements StructLayoutFactory {
    ...
    protected long calcNextOffset(Alignment currentAlignment, long currentSizeInBytes) {
        if ((structAlignment == null) || (currentAlignment.alignof > structAlignment.alignof)) {
            throw new IllegalArgumentException("current Alignment is too big for the structure");
        }
     ...
     * @param mem
     * @param startOffset the possible offset wher to place the start of the
     * structure
     */
    public StructLayoutOnTheFlyFactory(Alignment structAlignment, AbstractNativeMemory mem, long startOffset) {
        this.structAlignment = structAlignment;
        nextOffset = AbstractNativeMemory.calcOffsetForAlignemt(mem, structAlignment, startOffset);
    }

    public long union(Alignment alignment, long sizeInBytes) {
        return calcNextOffset(alignment, sizeInBytes);
    }

    public long struct(Alignment alignment, long sizeInBytes) {
        return calcNextOffset(alignment, sizeInBytes);
    }

    public long array(Alignment alignment, int elementSize, int length) {
        return calcNextOffset(alignment, elementSize * length);
    }
