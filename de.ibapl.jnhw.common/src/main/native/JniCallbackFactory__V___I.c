...
#define TRAMPOLINE(index) \
    ...
    void _jnhw_trampoline__V___I__ ## index (int32_t value) {\
        ...
        if ((callback_refs.callbackClassRef == NULL) || (callback_refs.trampolineMethodID == 0)) { \
            jnhw_log_stderr("unassigned Callback__V___I %d @0x%08lx no upcall handle", index, (uint64_t)(uintptr_t)&_jnhw_trampoline__V___I__ ## index); \
            return; \
        } \
        JNIEnv *env;\
        if ((*common_jvm)->AttachCurrentThread(common_jvm, (void**) &env, NULL)) {\
            jnhw_log_stderr("Callback__V___I %d @0x%08lx can't attach to thread", index, (uint64_t)(uintptr_t)&_jnhw_trampoline__V___I__ ## index); \
            return;\
        } else {\
            if (*env == NULL) {\
            jnhw_log_stderr("Callback__V___I %d @0x%08lx can't get env", index, (uint64_t)(uintptr_t)&_jnhw_trampoline__V___I__ ## index); \
                return;\
            }\
