...
#if defined(__LP64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V___I %d @0x%016lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V___I %d @0x%016lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V___I %d @0x%016lx can't get env!\n"
#elif defined(__WIN64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V___I %d @0x%016Lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V___I %d @0x%016Lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V___I %d @0x%016Lx can't get env!\n"
#else 
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V___I %d @0x%08x no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V___I %d @0x%08x can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V___I %d @0x%08x can't get env!\n"
#endif 
...
#define TRAMPOLINE(index) \
    ...
    void _jnhw_trampoline__V___I__ ## index (int32_t value) {\
        ...
        if ((callback_refs.callbackClassRef == NULL) || (callback_refs.trampolineMethodID == 0)) { \
            jnhw_log_stderr(MSG_UNASSIGNED, index, (uintptr_t)&_jnhw_trampoline__V___I__ ## index); \
            return; \
        } \
        JNIEnv *env;\
        if ((*common_jvm)->AttachCurrentThread(common_jvm, (void**) &env, NULL)) {\
            jnhw_log_stderr(MSG_ATTACH_THREAD_FAILED, index, (uintptr_t)&_jnhw_trampoline__V___I__ ## index); \
            return;\
        } else {\
            if (*env == NULL) {\
            jnhw_log_stderr(MSG_NO_UPCALL, index, (uintptr_t)&_jnhw_trampoline__V___I__ ## index); \
                return;\
            }\
