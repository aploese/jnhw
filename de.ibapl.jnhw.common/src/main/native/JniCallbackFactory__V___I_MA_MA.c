...
#include "de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory__V___I_MA_MA.h"
...
#if defined(__LP64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V___I_MA_MA %d @0x%016lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V___I_MA_MA %d @0x%016lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V___I_MA_MA %d @0x%016lx can't get env!\n"
#elif defined(__WIN64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V___I_MA_MA %d @0x%016Lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V___I_MA_MA %d @0x%016Lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V___I_MA_MA %d @0x%016Lx can't get env!\n"
#else 
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V___I_MA_MA %d @0x%08x no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V___I_MA_MA %d @0x%08x can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V___I_MA_MA %d @0x%08x can't get env!\n"
#endif 

#define TRAMPOLINE(index) \
    static struct cb_refs callback_refs_ ## index; \
    void _jnhw_trampoline__V___I_MA_MA__ ## index (int32_t a, void* b, void* c) {\
        ...
        if ((callback_refs.callbackClassRef == NULL) || (callback_refs.trampolineMethodID == 0)) { \
            jnhw_log_stderr(MSG_UNASSIGNED, index, (uintptr_t)&_jnhw_trampoline__V___I_MA_MA__ ## index); \
            return; \
        } \
        JNIEnv *env;\
        if ((*common_jvm)->AttachCurrentThread(common_jvm, (void**) &env, NULL)) {\
            jnhw_log_stderr(MSG_ATTACH_THREAD_FAILED, index, (uintptr_t)&_jnhw_trampoline__V___I_MA_MA__ ## index); \
            return;\
        } else {\
            if (*env == NULL) {\
            jnhw_log_stderr(MSG_NO_UPCALL, index, (uintptr_t)&_jnhw_trampoline__V___I_MA_MA__ ## index); \
                return;\
            }\
            (*env)->CallStaticVoidMethod(env, callback_refs.callbackClassRef, callback_refs.trampolineMethodID, a, (int64_t)(uintptr_t)b, (int64_t)(uintptr_t)c);\
...
TRAMPOLINE(15)
    ...
    /*
     * Class:     de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory__V___I_MA_MA
     * Method:    registerCallBack
     * Signature: (ILjava/lang/Class;Ljava/lang/String;)J
     */
    JNIEXPORT jlong JNICALL Java_de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory_1_1V_1_1_1I_1MA_1MA_registerCallBack
    (JNIEnv *env, __attribute__ ((unused)) jclass clazz, jint index, jclass callbackClass, jstring callBackMethodName) {
        ...
        const jmethodID trampolineMethodID = (*env)->GetStaticMethodID(env, callbackClassRef, _callBackMethodName , "(IJJ)V");
...
#define TRAMPOLINE_REGISTER_CASE(index) case index: \
                ...
                return (int64_t)(uintptr_t)&_jnhw_trampoline__V___I_MA_MA__ ## index;
     ...
     * Signature: (I)I
     */
    JNIEXPORT jint JNICALL Java_de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory_1_1V_1_1_1I_1MA_1MA_releaseCallBack
...
#define TRAMPOLINE_RELEASE_CASE(index) case index: \
     ...
     * Signature: ()I
     */
    JNIEXPORT jint JNICALL Java_de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory_1_1V_1_1_1I_1MA_1MA_getMaxCallBacks
