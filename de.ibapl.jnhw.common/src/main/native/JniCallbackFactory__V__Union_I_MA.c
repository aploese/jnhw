...
#include "de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory__V__Union_I_MA.h"
...
#if defined(__LP64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V__Union_I_MA %d @0x%016lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V__Union_I_MA %d @0x%016lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V__Union_I_MA %d @0x%016lx can't get env!\n"
#elif defined(__WIN64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V__Union_I_MA %d @0x%016Lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V__Union_I_MA %d @0x%016Lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V__Union_I_MA %d @0x%016Lx can't get env!\n"
#else 
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V__Union_I_MA %d @0x%08x no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V__Union_I_MA %d @0x%08x can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V__Union_I_MA %d @0x%08x can't get env!\n"
#endif 

#define TRAMPOLINE(index) \
    static struct cb_refs callback_refs_ ## index; \
    void _jnhw_trampoline__V__Union_I_MA__ ## index (union Union_I_MA  value) {\
        ...
        if ((callback_refs.callbackClassRef == NULL) || (callback_refs.trampolineMethodID == 0)) { \
            jnhw_log_stderr(MSG_UNASSIGNED, index, (uintptr_t)&_jnhw_trampoline__V__Union_I_MA__ ## index); \
            return; \
        } \
        JNIEnv *env;\
        if ((*common_jvm)->AttachCurrentThread(common_jvm, (void**) &env, NULL)) {\
            jnhw_log_stderr(MSG_ATTACH_THREAD_FAILED, index, (uintptr_t)&_jnhw_trampoline__V__Union_I_MA__ ## index); \
            return;\
        } else {\
            if (*env == NULL) {\
            jnhw_log_stderr(MSG_NO_UPCALL, index, (uintptr_t)&_jnhw_trampoline__V__Union_I_MA__ ## index); \
                return;\
            }\
            (*env)->CallStaticVoidMethod(env, callback_refs.callbackClassRef, callback_refs.trampolineMethodID, (int64_t)(uintptr_t)value.value_ptr, value.value_int);\
...
/*
 * Class:     de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory__V__Union_I_MA
 * Method:    registerCallBack
 * Signature: (ILjava/lang/Class;Ljava/lang/String;)J
 */
JNIEXPORT jlong JNICALL Java_de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory_1_1V_1_1Union_1I_1MA_registerCallBack
    (JNIEnv *env, __attribute__ ((unused)) jclass clazz, jint index, jclass callbackClass, jstring callBackMethodName) {
        ...
        const jmethodID trampolineMethodID = (*env)->GetStaticMethodID(env, callbackClassRef, _callBackMethodName , "(JI)V");
...
#define TRAMPOLINE_REGISTER_CASE(index) case index: \
                ...
                return (int64_t)(uintptr_t)&_jnhw_trampoline__V__Union_I_MA__ ## index;
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory_1_1V_1_1Union_1I_1MA_releaseCallBack
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory_1_1V_1_1Union_1I_1MA_getMaxCallBacks
