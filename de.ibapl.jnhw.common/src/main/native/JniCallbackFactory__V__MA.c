...
#include "de_ibapl_jnhw_common_upcall_jni_JniCallbackFactory__V__MA.h"
...
#if defined(__LP64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V__MA %d @0x%016lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V__MA %d @0x%016lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V__MA %d @0x%016lx can't get env!\n"
#elif defined(__WIN64__)
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V__MA %d @0x%016Lx no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V__MA %d @0x%016Lx can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V__MA %d @0x%016Lx can't get env!\n"
#else 
 #define MSG_UNASSIGNED "From C code: unassigned Callback__V__MA %d @0x%08x no upcall handle!\n"
 #define MSG_ATTACH_THREAD_FAILED "From C code: Callback__V__MA %d @0x%08x can't attach to thread!\n"
 #define MSG_NO_UPCALL "From C code: Callback__V__MA %d @0x%08x can't get env!\n"
#endif 
...
TRAMPOLINE(15)
    ...
    (JNIEnv *env, __attribute__ ((unused)) jclass clazz, jint index, jclass callbackClass, jstring callBackMethodName) {
        ...
        const jmethodID trampolineMethodID = (*env)->GetStaticMethodID(env, callbackClassRef, _callBackMethodName , "(J)V");
...
#define TRAMPOLINE_REGISTER_CASE(index) case index: \
                ...
                return (int64_t)(uintptr_t)&_jnhw_trampoline__V__MA__ ## index;
