 ...
 * @author aploese
 */
public class Callback_I_I_PtrOpaquememory_V_Test {

    static class C extends OpaqueMemory {
    ...
    public Callback_I_I_PtrOpaquememory_V_Test() {
    }

    private static native NativeFunctionPointer getCallbackPtr();

    private static native void setCallback(Callback_I_I_PtrOpaqueMemory_V<C> callback);
    ...
    private class DummyCB extends Callback_I_I_PtrOpaqueMemory_V_Impl {
        ...
        @Override
        protected OpaqueMemory wrapC(NativeAddressHolder address) {
            throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        }
    ...
    public void testMAX_CALL_BACKS() {
        ...
        int maxCB = Callback_I_I_PtrOpaqueMemory_V_Impl.MAX_CALL_BACKS();
        assertEquals(8, maxCB);
        Callback_I_I_PtrOpaqueMemory_V_Impl[] cbs = new Callback_I_I_PtrOpaqueMemory_V_Impl[maxCB];
        for (int i = 0; i < cbs.length; i++) {
            cbs[i] = new DummyCB();
            assertEquals(maxCB - (i + 1), Callback_I_I_PtrOpaqueMemory_V_Impl.callbacksAvailable());
        }
        ...
        assertEquals(maxCB, Callback_I_I_PtrOpaqueMemory_V_Impl.callbacksAvailable());
    }
    ...
    public void testReleaseByGarbageCollector() {
        ...
        assertSame(Callback_I_I_PtrOpaqueMemory_V_Impl.find(getCallbackPtr()), callback);
        ...
        assertEquals(Callback_I_I_PtrOpaqueMemory_V_Impl.MAX_CALL_BACKS(), Callback_I_I_PtrOpaqueMemory_V_Impl.callbacksAvailable());
