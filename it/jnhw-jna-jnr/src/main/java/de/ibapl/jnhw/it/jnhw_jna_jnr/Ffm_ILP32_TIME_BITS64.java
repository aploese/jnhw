 ...
 * @author aploese
 */
public class Ffm_ILP32_TIME_BITS64 {

    final static boolean DIRECT_ACCESS = true;

    final static ValueLayout TV_SEC_LAYOUT = ValueLayout.JAVA_LONG.withName("tv_sec");
    final static ValueLayout TV_NSEC_MEMORY_LAYOUT = ValueLayout.JAVA_INT.withName("tv_nsec");
    ...
    final static MethodHandle clock_gettime = LINKER.downcallHandle(
            SYMBOL_LOOKUP.find("__clock_gettime64").get(),
    ...
    final static MethodHandle clock_settime = LINKER.downcallHandle(
            SYMBOL_LOOKUP.find("__clock_settime64").get(),
            FunctionDescriptor.of(ValueLayout.JAVA_LONG, ValueLayout.JAVA_INT, ValueLayout.ADDRESS)
    );
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (DIRECT_ACCESS) {
                for (int i = 0; i < count; i++) {
                    ...
                    timespec.set(ValueLayout.JAVA_LONG, Time.Timespec.offsetof_Tv_sec, timespec.get(ValueLayout.JAVA_INT, Time.Timespec.offsetof_Tv_nsec));
                    timespec.set(ValueLayout.JAVA_INT, Time.Timespec.offsetof_Tv_nsec, (int) value);
                }
            } else {
                for (int i = 0; i < count; i++) {
                    MemorySegment timespec = heap.asSlice(0, de.ibapl.jnhw.posix.Time.Timespec.sizeof);
                    ...
                    tv_nsec.set(timespec, (int) value);
                }
            }
        } catch (Throwable th) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (DIRECT_ACCESS) {
                for (int i = 0; i < count; i++) {
                    ...
                    val = timespec.get(ValueLayout.JAVA_INT, Time.Timespec.offsetof_Tv_nsec);
                }
            } else {
                for (int i = 0; i < count; i++) {
                    val = ((long) tv_sec.get(timespec));
                    val = ((int) tv_nsec.get(timespec));
                }
            }
        }
    }
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (DIRECT_ACCESS) {
                for (int i = 0; i < count; i++) {
                    ...
                    timespec.set(ValueLayout.JAVA_INT, Time.Timespec.offsetof_Tv_nsec, (int) val);
                }
            } else {
                for (int i = 0; i < count; i++) {
