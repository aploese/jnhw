/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class de_ibapl_jnhw_posix_SignalTest_NativeMcontext_t */

#ifndef _Included_de_ibapl_jnhw_posix_SignalTest_NativeMcontext_t
#define _Included_de_ibapl_jnhw_posix_SignalTest_NativeMcontext_t
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     de_ibapl_jnhw_posix_SignalTest_NativeMcontext_t
 * Method:    alignof
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_de_ibapl_jnhw_posix_SignalTest_00024NativeMcontext_1t_alignof
  (JNIEnv *, jclass);

/*
 * Class:     de_ibapl_jnhw_posix_SignalTest_NativeMcontext_t
 * Method:    sizeof
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_de_ibapl_jnhw_posix_SignalTest_00024NativeMcontext_1t_sizeof
  (JNIEnv *, jclass);

#ifdef __cplusplus
}
#endif
#endif
